...
#
# consolidate (Stage 6) votes over whole-bar units, and a mid-bar boundary does
# NOT move the bar grid the pattern repeats on -- a 1-bar drum figure still
# repeats every bar whichever beat the section started on. So the only question
# is which bars belong wholly to which section, and a partial bar belongs to
# neither: voting it into either neighbour mixes two patterns in one slot.


def test_spans_are_whole_bars(song):
    song.bars = 40
    song.sections = [Section("a", 1), Section("b", 9), Section("c", 17)]
    assert song.consolidation_spans() == [("a", 1, 9), ("b", 9, 17), ("c", 17, 41)]


def test_a_section_starting_mid_bar_gives_up_its_partial_first_bar(song):
    """Manlio's verse-2 starts at 20.3, so bar 20 is half break and half verse
    and belongs to neither. Its first whole bar is 21."""
    song.bars = 40
    song.sections = [Section("break-1", 18), Section("verse-2", 20, beat=3.0)]
    spans = dict((n, (a, b)) for n, a, b in song.consolidation_spans())
    assert spans["break-1"] == (18, 20)      # bars 18 and 19, not 20
    assert spans["verse-2"] == (21, 41)


def test_a_section_too_short_to_hold_a_whole_bar_is_dropped(song):
    """Better absent than voted on with a fabricated span -- consolidate would
    pass those hits through untouched, which is what Stage 7 wants anyway."""
    song.bars = 40
    song.sections = [Section("a", 1), Section("blink", 9, beat=3.0),
                     Section("b", 10)]
    names = [n for n, _, _ in song.consolidation_spans()]
    assert "blink" not in names
    assert names == ["a", "b"]


def test_the_spans_never_overlap_and_stay_in_order(song):
    song.bars = 80
    song.sections = [
        Section("intro", 1), Section("verse-1", 6), Section("break-1", 18),
    ...
    for (_, _, end), (_, start, _) in zip(spans, spans[1:], strict=False):
        assert start >= end, "a bar must not be voted into two sections"
    assert [n for n, _, _ in spans] == [s.name for s in
                                        sorted(song.sections, key=lambda x: x.position)]
